# terraform/aws Terraform read-only AWS sample design
...
`terraform/aws` is currently a minimal Bun package with:
...
- Put Terraform files under `terraform/aws/`.
...
Given a user opens `terraform/aws/README.md`, when they follow the
...
- Use read-only `aws_caller_identity` instead of a resource.
  ...
  - Confidence: 88%.
- Place Terraform under `terraform/aws/`.
