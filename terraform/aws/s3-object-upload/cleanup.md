...
- `terraform/aws/s3-object-upload/terraform.tfvars` に、作成時と同じ `bucket_name` / `object_key` が残っていること
...
```bash
mise exec -- terraform -chdir=terraform/aws/s3-object-upload state list
```
