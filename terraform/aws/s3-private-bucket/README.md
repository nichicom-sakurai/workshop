...
```bash
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket plan -var='bucket_prefix=nck-sakurai-tf-learn-01'
```
...
```bash
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket init
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket fmt -check
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket validate
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket plan      # 作成内容を確認
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket apply     # bucket を1つ作成
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket state list # state 上のリソースを確認
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket destroy   # 学習後に削除
```
...
```bash
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket plan -destroy
```

確認後、同じ root module / state で `destroy` します。

```bash
mise exec -- terraform -chdir=terraform/aws/s3-private-bucket destroy
```
