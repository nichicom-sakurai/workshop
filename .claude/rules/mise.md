...
- `mise.toml` is the single source of truth for tool versions. Never hardcode versions in docs or scripts.
  - Exception: Dockerfiles can't read `mise.toml`, so they pin the same version explicitly (e.g. `packages/gc/apps/cloud-run-rest/Dockerfile` ↔ `[tools]` `bun`). When bumping a tool version here, update the matching Dockerfile `FROM` tag together.
