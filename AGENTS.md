...
- **Learning skeleton + read-only Terraform samples.** Each of `packages/aws/` and `packages/gc/` keeps its `package.json` + `index.ts` skeleton (`console.log("Hello from <name>")`) and adds a self-contained, read-only Terraform sample: `packages/aws/terraform/<operation>/` (first: `caller-identity`) and `packages/gc/terraform/<operation>/` (first: `project-info`, reads Google Cloud project `nck-sakurai`).
...
- `packages/*` — the projects. Targets of `bun install` / `dev` / `dev:all`. Each is independent (no root npm workspaces). Currently `aws` and `gc`; `aws` contains `terraform/<operation>/` — one independent Terraform root module per AWS operation (first: `caller-identity`, read-only) — and `gc` contains `terraform/<operation>/` (first: `project-info`, a read-only Google Cloud project check for `nck-sakurai`).
...
- `packages/aws/terraform/<operation>/` — each AWS operation is a self-contained, independent root module (own state) placed directly under `terraform/`. First sample: `caller-identity`, which uses `data "aws_caller_identity" "current" {}` only.
...
- **Terraform samples are read-only.** `packages/aws/terraform/<operation>/` holds one independent root module per AWS operation (today only `caller-identity`); `packages/gc/terraform/<operation>/` does the same per Google Cloud operation (today only `project-info`, reading the project `nck-sakurai`). There are no resource-creating `.tf` files, no remote backend, and bootstrap does no provisioning. Future mutating samples follow the same flat `terraform/<operation>/` layout — distinguished by the README's 種別 column, not a separate tree; ask first before adding resource-creating samples.
