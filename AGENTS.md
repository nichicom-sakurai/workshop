...
- **Learning skeleton + Terraform samples.** Each of `packages/aws/` and `packages/gc/` keeps its `package.json` + `index.ts` skeleton (`console.log("Hello from <name>")`) and adds self-contained Terraform samples under `packages/<provider>/terraform/<operation>/`. AWS starts with `caller-identity` (read-only), `s3-private-bucket` (mutating), `s3-object-upload` (mutating), and `agentcore-runtime-basic` (mutating, backed by a nested Python app under `packages/aws/apps/`); Google Cloud starts with `project-info`, expands through storage learning samples, and adds Cloud Run (`cloud-run-service-basic`) and Vertex AI Agent Engine (`adk-agent-engine-basic`, prerequisite API via `vertex-ai-api-enable`) deploy samples backed by nested apps under `packages/gc/apps/` (which also holds `adk-helloworld`, a Google ADK HelloWorld Python app run locally via `adk run` / `adk web` that also generates the inline-source archive deployed by `adk-agent-engine-basic`). Beyond Terraform, AWS also ships `packages/aws/cost-estimator/` — a non-Terraform, estimation-only YAML catalog (web/database/container/functions/iam) plus a `bcm-pricing-calculator` API adapter, zero **runtime** deps (`@types/bun` dev-only; YAML via `Bun.YAML`, tests via `bun:test`).
- **OpenAI Agents SDK sample.** `packages/openai/` is a third top-level package — an OpenAI Agents SDK (TypeScript) HelloWorld. Unlike the `aws`/`gc` skeletons it carries real exact-pinned deps (`@openai/agents`, `zod`) + a committed `bun.lock` + a `tsconfig.json` (+ `@types/bun`) for `bunx tsc --noEmit`; `index.ts` prints setup guidance and exits 0 when `OPENAI_API_KEY` is unset (so `dev:all` stays green), and runs one `Agent` / `run` call printing `finalOutput` when the key is set. See [`packages/openai/README.md`](./packages/openai/README.md).
- **[WARNING] no repo-wide test / lint / typecheck.** `packages/gc/apps/cloud-run-rest/`, `packages/aws/cost-estimator/`, and `packages/openai/` each carry a `tsconfig.json` (+ `@types/bun`) for `bunx tsc --noEmit` (and `cost-estimator` adds `bun test`); `packages/aws/apps/agentcore-strands-basic/` and `packages/gc/apps/adk-helloworld/` each carry `pyproject.toml` + `uv.lock` and verify with `uv run ... python -m unittest`. Everything else runs untyped on Bun. Terraform exists as learning samples with local state only; no remote backend is configured. See "Gotchas".
...
- `packages/*` — the projects. Targets of `bun install` / `dev` / `dev:all`. Each is independent (no root npm workspaces). Currently `aws`, `gc`, and `openai`. The provider packages `aws` and `gc` each contain `terraform/<operation>/` — one independent Terraform root module per cloud operation (`openai` has none; it is a runnable SDK sample, detailed below). AWS includes `caller-identity` (read-only), `s3-private-bucket` (mutating), `s3-object-upload` (mutating), and `agentcore-runtime-basic` (mutating, deploys the nested app below); Google Cloud includes `project-info`, storage learning samples, `cloud-run-service-basic` (mutating, deploys the nested app below), and `adk-agent-engine-basic` (mutating, deploys `adk-helloworld` to Vertex AI Agent Engine; prerequisite API via `vertex-ai-api-enable`).
- `packages/aws/apps/<app>/` — deployable app code used by AWS Terraform samples (first: `agentcore-strands-basic`, a Python 3.13 + uv + Strands Agents app packaged as an AgentCore Runtime direct code deployment ZIP). Nested apps are **not** picked up by `dev:all`; `agentcore-strands-basic` is verified with `mise exec -- uv run --directory packages/aws/apps/agentcore-strands-basic --locked python -m unittest discover -s tests` and packaged with `packages/aws/apps/agentcore-strands-basic/scripts/package.sh`.
- `packages/gc/apps/<app>/` — deployable / runnable app code for Google Cloud learning. Two apps today: `cloud-run-rest` (a Bun REST service — zero **runtime** deps, `@types/bun` dev-only — plus a Dockerfile for the `cloud-run-service-basic` Terraform sample) and `adk-helloworld` (a Python 3.13 + uv Google ADK HelloWorld agent whose package `hello_world/` exposes `root_agent`, run locally via `adk run` / `adk web`; it also backs the `adk-agent-engine-basic` Terraform sample by generating an inline-source archive via `scripts/package-agent-engine.sh`, whose `.build/` output is gitignored, with the Agent-Engine-only files under `agent-engine/`). Bun apps use `src/index.ts` as the entry point; the Python app uses `hello_world/agent.py`. Nested apps are **not** picked up by `dev:all` (its loop covers `packages/*/` only; `dev <name>` is documented for top-level packages); bootstrap's recursive `find` still runs `bun install` in them (a real install for `cloud-run-rest`, fetching `@types/bun`; a no-op for `adk-helloworld`, which has no `package.json`).
...
- `packages/openai/` — a top-level **runnable** OpenAI Agents SDK (TypeScript) HelloWorld (not a skeleton). `index.ts` exits 0 with setup guidance when `OPENAI_API_KEY` is unset (keeping `dev:all` green) and otherwise runs one `Agent` / `run` call printing `finalOutput`. Carries exact-pinned `@openai/agents` + `zod`, a committed `bun.lock`, and a `tsconfig.json` (+ `@types/bun`) for `bunx tsc --noEmit` (its `moduleDetection: "force"` makes the dynamic-import `index.ts` a module so top-level `await` type-checks). `OPENAI_DEFAULT_MODEL` overrides the SDK default model. Also ships a terminal chat (`mise run chat openai` → `chat.ts`) and a minimal Bun.serve web chat UI (`mise run web openai` → `web.ts` + `chat.html`, streams responses), both multi-turn via the SDK's `run()` history. See [`packages/openai/README.md`](./packages/openai/README.md).
...
| Run one project | `mise run dev <name>` | `<name>` is `aws` / `gc` / `openai` |
| Run all projects | `mise run dev:all` | loops over `packages/` |
| Interactive chat | `mise run chat <name>` | multi-turn terminal chat; only `openai` has a `chat` script today |
| Web chat UI | `mise run web <name>` | minimal Bun.serve + HTML streaming chat; only `openai` has a `web` script today |
...
- `packages/aws/terraform/<operation>/` — each AWS operation is a self-contained, independent root module (own state) placed directly under `terraform/`. Current samples include `caller-identity` (read-only, uses `data "aws_caller_identity" "current" {}` only), `s3-private-bucket` (mutating, creates a private S3 bucket and public access block), `s3-object-upload` (mutating, uploads a local file to an existing S3 bucket), and `agentcore-runtime-basic` (mutating, creates S3 artifact storage, IAM, AgentCore Runtime, and a custom endpoint from a prebuilt ZIP).
...
- Integration: `mise run dev:all` runs all projects (aws / gc / openai) without crashing.
...
- `packages/gc/apps/cloud-run-rest/` and `packages/openai/` each have a `tsconfig.json` + `@types/bun`, so `mise exec -- bunx tsc --noEmit` (run from that dir) typechecks them today. No repo-wide typecheck task is wired into `mise.toml` yet.
- `packages/aws/apps/agentcore-strands-basic/` has a `pyproject.toml` + `uv.lock`; run `mise exec -- uv lock --directory packages/aws/apps/agentcore-strands-basic --check`, `mise exec -- uv run --directory packages/aws/apps/agentcore-strands-basic --locked python -m unittest discover -s tests`, and `packages/aws/apps/agentcore-strands-basic/scripts/package.sh` for its local verification.
- `packages/gc/apps/adk-helloworld/` has a `pyproject.toml` + `uv.lock`; run `mise exec -- uv lock --directory packages/gc/apps/adk-helloworld --check` and `mise exec -- uv run --directory packages/gc/apps/adk-helloworld --locked python -m unittest discover -s tests` for its local verification (the agent itself runs via `adk run` / `adk web` — see its README). The unittest suite also includes `test_agent_engine_packaging.py`, a stdlib-only guard that the Agent Engine `requirements.txt` `google-adk` pin stays in sync with `pyproject.toml` and that `agent-engine/agent_engine_app.py` wraps `root_agent` in `AdkApp`; `scripts/package-agent-engine.sh` builds the Agent Engine deploy archive (output under the gitignored `.build/`).
...
- **Lockfile / tsconfig / pyproject: skeleton packages have none.** Skeleton packages have no deps (no `bun.lock`) and bun runs TypeScript / ESM natively without a `tsconfig.json`, so `index.ts` runs directly with no typecheck step. The TypeScript exceptions are `packages/gc/apps/cloud-run-rest/` and `packages/aws/cost-estimator/`: each carries `@types/bun`, a committed `bun.lock` (repeatable installs, like Terraform's committed `.terraform.lock.hcl`), and a `tsconfig.json` enabling `bunx tsc --noEmit` (`cost-estimator` also adds `bun test` and `exactOptionalPropertyTypes`). `packages/openai/` is a third TypeScript case: a top-level package carrying exact-pinned `@openai/agents` + `zod`, a committed `bun.lock`, `@types/bun`, and a `tsconfig.json` whose `moduleDetection: "force"` makes the dynamic-import `index.ts` a module (so top-level `await` type-checks and `bunx tsc --noEmit` passes). The Python exceptions are `packages/aws/apps/agentcore-strands-basic/` and `packages/gc/apps/adk-helloworld/`, which each carry `pyproject.toml` + `uv.lock` and use package-local `uv` / `unittest` checks. These are still package-local — don't assume a repo-wide typecheck exists.
