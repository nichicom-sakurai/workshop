...
- **OpenAI Agents SDK sample.** `packages/openai/` is a third top-level package — an OpenAI Agents SDK (TypeScript) HelloWorld. Unlike the `aws`/`gc` skeletons it carries real exact-pinned deps (`@openai/agents`, `zod`) + a committed `bun.lock`; `index.ts` prints setup guidance and exits 0 when `OPENAI_API_KEY` is unset (so `dev:all` stays green), and runs one `Agent` / `run` call printing `finalOutput` when the key is set. See [`packages/openai/README.md`](./packages/openai/README.md).
- **[WARNING] no repo-wide test / lint / typecheck.** `packages/gc/apps/cloud-run-rest/` and `packages/aws/cost-estimator/` each carry a `tsconfig.json` (+ `@types/bun`) for `bunx tsc --noEmit` (and `cost-estimator` adds `bun test`); `packages/aws/apps/agentcore-strands-basic/` and `packages/gc/apps/adk-helloworld/` each carry `pyproject.toml` + `uv.lock` and verify with `uv run ... python -m unittest`. Everything else runs untyped on Bun. Terraform exists as learning samples with local state only; no remote backend is configured. See "Gotchas".
...
- `packages/openai/` — a top-level **runnable** OpenAI Agents SDK (TypeScript) HelloWorld (not a skeleton). `index.ts` exits 0 with setup guidance when `OPENAI_API_KEY` is unset (keeping `dev:all` green) and otherwise runs one `Agent` / `run` call printing `finalOutput`. Carries exact-pinned `@openai/agents` + `zod` and a committed `bun.lock`; no `tsconfig.json` (runs untyped on Bun). `OPENAI_DEFAULT_MODEL` overrides the SDK default model. See [`packages/openai/README.md`](./packages/openai/README.md).
